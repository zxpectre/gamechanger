...
const encoder=async (obj:any,options:{urlPattern?:string,msgPlaceholder?:string,encodingOptions?:any,encoding?:APIEncoding})      =>{
  ...
  if(!parsedSolvedURL) //if dont pass URL validation check
      throw new Error("Failed to construct a valid URL with provided pattern and message");
  if(!solvedURL.startsWith(parsedSolvedURL.origin)) //(*) check if origin was overrided by a templating attack
...
const decoder=async (msg:string,options:{urlPattern?:string,msgPlaceholder?:string,encodingOptions?:any,encoding?:APIEncoding})      =>{
  const useUrlPattern=options?.urlPattern||"";
