import { APIEncoding, DefaultAPIEncodings, DefaultAPIVersion, EncodingHandler } from "../types";
 ...
 */
export const HeadersByEncoders:{[encoding:string]:string}=Object.fromEntries(
  Object.entries(EncodingByHeaders).map(([header,encoding])=>[encoding,header])
);

/**
 * Async loaders for the required encoding handlers, as a map.
 */
export const EncodingHandlers:{[name:string]:()=>Promise<EncodingHandler>}=Object.fromEntries(
...
const handler:EncodingHandler = { 
  ...
  encoder:async (obj:any,options?:{encodingOptions?:any,encoding?:APIEncoding})      =>{
    ...
    const msgBody=await handler.encoder(obj,options?.encodingOptions);
    const msg=`${['XQ','wo'].includes(header)?"":header}${msgBody}`;//legacy modes has no added header
    return msg;
},
  decoder:async (msg:string,options?:{encodingOptions?:any,encoding?:APIEncoding})   =>{
    if(!msg)
    ...
    const useMsg=!['XQ','wo'].includes(useHeader)//legacy modes has no header actually
        ...
        :msg;
    const obj=await handler.decoder(useMsg,options?.encodingOptions);
    return obj;
},
}
